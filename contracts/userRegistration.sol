...
contract UserRegistration {
    ...
    event newUser(uint256 id, string name, string description, UserType role);

    modifier checkRegistered(address _check) {
        require(
            alreadyRegistered[_check] == false,
    ...
    User[] public users;
    mapping(uint256 => address) profileToUser;
    mapping(address => uint256) userToProfile;
    ...
    function addUser(
        ...
        UserType _role
    ) external checkRegistered(msg.sender) {
        users.push(User(_name, _description, msg.sender, _role));
        uint256 id = uint32(users.length - 1);
    ...
    function viewUser(uint256 _userId)
        public
        view
        returns (
            string memory,
